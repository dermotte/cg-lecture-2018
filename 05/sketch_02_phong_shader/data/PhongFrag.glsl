...
void main() {
  ...
  vec3 reflectionDir = 2 * dot(-vertLightDir, vertNormal) * vertNormal + vertLightDir; // that's actually a reflection!
  vec4 spec = 0.2 * vec4(1.0, 1.0, 1.0, 1.0) * pow(max(dot(reflectionDir,-vertLightDir),0.0), 5) ;
